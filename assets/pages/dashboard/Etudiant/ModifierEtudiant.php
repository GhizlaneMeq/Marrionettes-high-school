...
if (isset($_POST['submit'])) {
    ...
    $query = "UPDATE `etudiants` SET
     `nom`='$name',`email`='$email',`phone`='$phone',`image`='$folder',`genre`='$genre',`level`='$level',`promotion`='$promotion' WHERE id = $id";
    $result = mysqli_query($connect, $query);

    if (isset($result)) {
        move_uploaded_file($temp_name, $folder);
        header("location:AfficherEtudiant.php?msg=Etudiant updated successfully");
    } else {
        echo ("error");
    }
}
